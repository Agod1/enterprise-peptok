...
export default function CreateCoachingRequest() {
  ...
  useEffect(() => {
    ...
    const draft = localStorage.getItem("coaching-request-draft");
    if (draft) {
      try {
  ...
  const handleSubmitRequest = async (data: CoachingRequestFormData) => {
    ...
    try {
      ...
      localStorage.removeItem("coaching-request-draft");
  ...
  const handleSaveDraft = async (data: CoachingRequestFormData) => {
    try {
      // Save to localStorage as backup
      localStorage.setItem("coaching-request-draft", JSON.stringify(data));
